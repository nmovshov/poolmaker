...
def _main():
    ...
    # Sort competitors by rank (NOTE: assumes correct rank code: '[A-Z]YY' or 'U')
    rkey = lambda f: f[-1]
    ...
    if ps is None:
        ...
        ps = _lmfactor(N,5,6)
    if ps is None:
        raise Exception("Failed to find valid pool sizes.")
...
def _fill_pools(fencers, pools, pool_sizes):
    # Deal fencers by rank (descending) into pools
